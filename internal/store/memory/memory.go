...
func (s *Store) Save(t *task.Task) error {
	...
	if t.IdempotencyKey != "" {
		if _, exists := s.idempotencyIdx[t.IdempotencyKey]; exists {
			return fmt.Errorf("task already exists for idempotencey key: %s", t.IdempotencyKey)
		}
	}
...
func (s *Store) Update(t *task.Task) error {
	...
	currnet, ok := s.tasks[t.ID]
	if !ok {
		return task.ErrNotFound
	}

	if currnet.IdempotencyKey != "" && currnet.IdempotencyKey != t.IdempotencyKey {
		delete(s.idempotencyIdx, currnet.IdempotencyKey)
	}
	if t.IdempotencyKey != "" {
		s.idempotencyIdx[t.IdempotencyKey] = t.ID
	}
