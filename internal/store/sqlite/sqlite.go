...
func ensureTaskColumns(db *sql.DB) error {
	...
	for _, mig := range migrations {
		...
		if _, err := db.Exec(mig.stmt); err != nil {
			return fmt.Errorf("migrste sqlite column %s: %w", mig.name, err)
		}
	}
...
func (s *Store) GetByIdempotencyKey(key string) (*task.Task, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("get task by idempotency key %s: %w", key, err)
	}
...
func (s *Store) Update(t *task.Task) error {
	...
	res, err := s.db.Exec(
		`UPDATE tasks
		SET payload = ?, status = ?, error = ?, retry_count = ?, max_retries = ?, next_run_at = ?, idempotency_key = ?, dead_lettered_at = ?, created_at = ?, updated_at = ?
		WHERE id = ?`,
		append([]byte(nil), t.Payload...),
		string(t.Status),
		t.Error,
