...
func TestPool_RetiriesFailedTask(t *testing.T) {
	store := memory.New()
	q := queue.NewWithStore(0, store, discardLog)
...
func TestPool_DeadLettersAfterRetriesExhausted(t *testing.T) {
	...
	waitForStatus(t, q, "bad-task", task.StatusDeadLettered, 2*time.Second)
...
func TestPool_TaskTimeoutSchedulesRetry(t *testing.T) {
	...
	waitForStatus(t, q, "timeout-task", task.StatusRetrying, 2*time.Second)
